class Equipment < ActiveRecord::Base
    ...
    scope :search_dates_available, -> (query) do
        # if query[:dates] && query[:dates][:pickup].present? && query[:dates][:dropoff].present?
        #     joins(:rentals).where("pickup_date >= ? AND dropoff_date <= ?", query[:dates][:pickup], query[:dates][:dropoff])  
        # end
    end
